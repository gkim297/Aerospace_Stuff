...
class Agent:
    ...
    def build_actor(self):
        ...
        agent_name = 'agent_' + str(self.__n_agent) # agent name 'agent_3', for example
        self.__state_placeholder = tf.placeholder(dtype = tf.float32, shape = [None, Settings.STATE_SIZE], name = 'state_placeholder') # the * lets Settings.STATE_SIZE be not restricted to only a scalar
    ...
    def run(self, stop_run_flag, replay_buffer_dump_flag, starting_episode_number):
        ...
        while episode_number <= Settings.NUMBER_OF_EPISODES and not stop_run_flag.is_set():
            ...
            if test_time and Settings.TEST_ON_DYNAMICS:
                self.__agent_to_env.put((True, test_time)) # Reset into a dynamics environment only if it's test time and desired
            else:
                self.__agent_to_env.put((False, test_time)) # Reset into a kinematics environment
            ...
            while not done:
                ...
                ##### Running the Policy #####
                ##############################
                action = self.__sess.run(self.__policy.action_scaled, feed_dict = {self.__state_placeholder: np.expand_dims(state,0)})[0] # Expanding the state to be a 1x3 instead of a 3
                ...
                next_state, reward, done, *guidance_position = self.__env_to_agent.get() # The * means the variable will be unpacked only if it exists
